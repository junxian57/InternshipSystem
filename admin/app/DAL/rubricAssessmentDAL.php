...
class rubricAssessmentDAL
{
     ...
     * @return bool
     */
    public function IsRubricExists($tiitle, $id, $assmtID, $facultyID)
    {
        $sql = "SELECT * FROM RubricAssessment WHERE Title='" . $tiitle . "' AND assessmentID <>'" . $assmtID . "'AND internshipBatchID = $id AND facultyID =$facultyID ";
        $result = $this->databaseConnectionObj->runQuery($sql);
