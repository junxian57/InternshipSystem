...
if (isset($_GET['assessmentID']) && isset($_GET['rubricAssessment']) == 'rubricAssessment') {
    ...
    $sql = "UPDATE  RubricAssessment SET status = 'activate' WHERE assessmentID = '$assessmentID'";
    try {
    ...
    $sql = "UPDATE  RubricComponentCriteria SET status = 'activate' WHERE criterionID = '$RubricCriteriaID'";
    try {
        $result = $db->executeQuery($sql);
    } catch (Exception $e) {
        echo json_encode($e);
