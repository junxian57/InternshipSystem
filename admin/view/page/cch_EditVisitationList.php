...
$aVisitationList = $visitationBllObj->GetvisitationList($_GET['id']);
...
<body class="cbp-spmenu-push">
        ...
        <div id="page-wrapper">
            <div class="main-page">
                ...
                if ($_GET['status'] == 'failed') {
                    echo "<script> warning('List cant be Updateded. Operation failed.');</script>";
                } elseif ($_GET['status'] == 'success') {
                    echo "<script> addSuccess('Edut visitation List successful'); </script>";
                } elseif ($visitationBllObj->errorMessage != '') {
                    echo "<script> warning('$visitationBllObj->errorMessage'); </script>";
                }

                ?><div class="forms ">

                    <h3 class="title1">Edit Visitation List</h3>
                    <div class="form-grids row widget-shadow" data-example-id="basic-forms">
                        ...
                        <div class="form-body">
                            <form method="post">
                                <div class="form-group col-md-3"> <label for="exampleInput">Visitation ID</label><input type="text" id="Visitation_CompanyID" name="Visitation_CompanyID" class="form-control" value="<?php echo $aVisitationList->getVisitation_CompanyID() ?>" readonly="readonly"></div>
    ...
    <script>
        ...
        async function fetchSelectedCompanyList() {
            const Visitation_CompanyID = document.getElementById("Visitation_CompanyID").value;
            const getVisitationListPhp = '../../app/DAL/ajaxGetSelectedCompanyList.php?Visitation_CompanyID=' + Visitation_CompanyID;
            let getVisitationListPhpRespond = await fetch(getVisitationListPhp);
            let VisitationListObj = await getVisitationListPhpRespond.json();
            return VisitationListObj;
        }

        async function InsertVisitationTable() {
            const cmpResult = await fetchvisitationCmpName();
            const cmpSelectedResult = await fetchSelectedCompanyList();
            ...
            if (cmpResult !== "No Data Found") {
                ...
                `;
                    if (cmpSelectedResult.length > count) {
                        if (cmpSelectedResult[count].companyID == cmpResult[i].companyID) {
                            trLeft.setAttribute("data-companyID", cmpResult[i].companyID);
                            trLeft.setAttribute("data-cmpName", cmpResult[i].cmpName);
                            trLeft.setAttribute("data-Title", cmpResult[i].cmpContactPerson);
                            trLeft.setAttribute("data-score", cmpResult[i].cmpCompanySize);
                    ...
                    <td>${cmpResult[i].cmpAddress}</td>
                    <td>
                        <input type="checkbox" data-companyID="${cmpResult[i].companyID}" name="${cmpResult[i].companyID}" class="tab-3-checkbox" checked>
                    </td>
                `;
                            count++;
                        }
        ...
        async function assign() {
            ...
            for (var i = 0; i < table.rows.length; i++) {
                ...
                `;
                        let url = `../../app/DAL/ajaxInsertSelectedVisitationCmp.php?Visitation_CompanyID=${Visitation_CompanyID}&cmpID=${table.rows[i].getAttribute('data-companyID')}&cmpName=${table.rows[i].getAttribute('data-cmpName')}`;
                        await fetch(url);
        ...
        async function InsertSelectedVisitationTable() {
            ...
            const cmpSelectedResult = await fetchSelectedCompanyList();
            const visitationTable = document.getElementById("selected-visitation-Company-List-table");
            const Visitation_CompanyID = document.getElementById("Visitation_CompanyID").value;

            if (visitationTable.hasChildNodes()) {
                removeAllChildNodes(visitationTable);
            }


            if (cmpSelectedResult !== "No Data Found") {

                for (let i = 0; i < cmpSelectedResult.length; i++) {
                    let trLeft = document.createElement("tr");

                    trLeft.setAttribute("data-companyID", cmpSelectedResult[i].companyID);
                    trLeft.setAttribute("data-cmpName", cmpSelectedResult[i].cmpName);
                    trLeft.setAttribute("data-Title", cmpSelectedResult[i].cmpContactPerson);
                    trLeft.setAttribute("data-score", cmpSelectedResult[i].cmpCompanySize);
                    trLeft.setAttribute("data-maxScore", cmpSelectedResult[i].cmpAddress);

                    trLeft.innerHTML = `
                    <td>${cmpSelectedResult[i].companyID}</td>
                    <td>${cmpSelectedResult[i].cmpName}</td>
                    <td>${cmpSelectedResult[i].cmpContactPerson}</td>
                    <td>${cmpSelectedResult[i].cmpCompanySize}</td>
                    <td>${cmpSelectedResult[i].cmpAddress}</td>
                    <td>
                    <button type="button" onClick="deleteFrmdb(this,${cmpSelectedResult[i].companyID});">
					<span class="glyphicon glyphicon-remove" aria-hidden="true"></span>
				    </button>
                ...
                `;
                    ...
                    visitationTable.appendChild(trLeft);
                }
            } else {
                //alert("No Data Found");
                return;
        ...
        async function deleteFrmdb(currentRow, cmpID) {
            ...
            if (confirm(text) == true) {
                ...
                let url = `../../app/DAL/ajaxDelSelectedVisitationCmp.php?Visitation_CompanyID=${Visitation_CompanyID}&cmpID=${cmpID.name}`;
                console.log(url);
