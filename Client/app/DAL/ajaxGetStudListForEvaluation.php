...
if (isset($_GET['companyID']) && isset($_GET['internshipBatchID'])) {
    ...
    $sql = "SELECT sr.studResultID ,s.studentID,s.studName,p.programmeName,c.cmpName, ij.jobTitle,sr.finalScore , ra.TotalWeight
    FROM Student s JOIN StudentResult sr ON s.studentID=sr.studentID JOIN Programme p on p.programmeID=s.programmeID JOIN InternApplicationMap iam on iam.studentID=s.studentID 
    JOIN InternJob ij on ij.internJobID = iam.internJobID JOIN Company c on c.companyID=ij.companyID JOIN RubricAssessment ra on ra.assessmentID = sr.assessmentID
