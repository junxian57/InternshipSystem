...
if(isset($_GET['internAppID'])){
  ...
  $query = "UPDATE InternApplicationMap SET appStudentFeedback ='Reject Offer', appStudRejectReason = '$appStudRejectReason' WHERE internAppID='$internAppID'";
  if ((mysqli_query($conn, $query))){
    ...
    if($success){
      ...
      echo "<script>window.open('xt-rejectOffer.php?declineOffer=$internAppID','_self')</script>";
    }
  }
}
		...
		<div id="page-wrapper">
			<div class="main-page">
              ...
              <div class="detailsimage">
                ...
                <h1 class="heading" >Reject Internship Offer</h1>
            	</div>
        		</div>
					</section>

					<section class="cmpdetails" id="cmpdetails">
						<h1 class="heading"> <span>Internship Offer</span> Info </h1>
              ...
              if(($appStudFeedback <> 'Accept Offer') && ($appStudFeedback <> 'Reject Offer')){
                echo "<div class='button-group'>
                        <button class='backBtn'><a href='xt-studOfferList.php'>Back</a></button>
              	        <button type='submit' class='rejectOffer' id='rejectOffer'>Reject</button>
                      </div>";
              }elseif ($appStudFeedback == 'Accept Offer'){
                echo "<div class='button-group'>
                        <button class='backBtn'><a href='xt-studOfferList.php'>Back</a></button>
                      </div>";
              }else{
  ...
  <div class="rejectForm">
    <div class="formContent">
      <div class="formWidget">
        <h1 id="heading1" class="reject-header title">Reject Offer - <?php echo $cmpName; ?></h1>
        <div class="closeR">+</div>
      </div> 
      <form id="rejectForm" method="POST" action="xt-rejectOffer.php?internAppID=<?php echo $internAppID; ?>">
        <textarea id="reason" name="appStudRejectReason" rows="4" placeholder="Reason of Reject Internship Offer*" required></textarea>
	...
	<script>
    document.getElementById('rejectOffer').addEventListener('click',
      function(){
